;(function($){
	...
	*/
	...
	$doc.on('floor-loadItem',function(ev,index,elem,success){
		...
		/*var $elevitems = $('.elev-floor-item');
		setTimeout(function(){
			$elevitems.siblings().removeClass('elevator-active');
			$elevitems.eq(index).addClass('elevator-active');
		},500)*/
		//请求数据
		getDataOnce($doc,'data/floor/floorData.json',function(floorData){
			var html = buildFloorHtml(floorData[index]);
			// $elevitems.eq(index).toggle('elevator-active');
			//模拟网络延时
			setTimeout(function(){
				$elem.html(html);
				//加载图片
	...
	/*function elevatorLoad(elem){
		$elem = $(elem);
		console.log()
		$elem.trigger('floor-loadItem');
	}
	elevatorLoad();*/
